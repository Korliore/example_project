...
router = APIRouter(prefix="/customerorder", 
                   tags=["customerorder"])

@router.get("/get/{id}")
async def get_order(request: Request, id: int = Path(...)):
   """Получить заказ покупателя по иду"""
   all_data = await BuyersOrderRepository(request).find_one({"id": id})
   if all_data:
       return all_data
   raise HTTPException(status_code=400, detail=f"Customerorder with id {id} not found")

@router.get("/get_filter")
async def get_order(request: Request, filter_by: crm.CustomerOrderFilter = Depends(), offset: int = 0, limit: int = 50):
    """Получить все заказы покупателя по фильтрам"""
    if limit > 1000:
        raise HTTPException(status_code=400, detail=f"Max limit = 1000")
    filter_by = await clear_data(dict_=filter_by.dict())
    all_data = await BuyersOrderRepository(request).find_all(filter_by = filter_by, offset=offset, limit=limit)
    if all_data:
        return all_data
    raise HTTPException(status_code=400, detail=f"Orders not found")

@router.post("/add")
async def add_order(request: Request, insert_params: crm.CustomerOrderCreate = Body(...)) -> None:
    ...
    all_data = await BuyersOrderRepository(request).insert_one(data=insert_params_clear)  # Передача request в репозиторий
    return all_data